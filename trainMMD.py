...
def sample_image(batches_done):
    ...
    # 保存图片
    save_image(generated_imgs.data, "./imagesMMD/%d.png" % batches_done, nrow=10, normalize=True)
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, _) in enumerate(all_dataloader):
            ...
            z_fake = Variable(torch.randn(imgs.size()[0], 8) * opt.sigma)
            if torch.cuda.is_available():
                z_fake = z_fake.cuda()

            # z_real = encoder(imgs)

            mmd_loss = imq_kernel(z, z_fake, h_dim=8)
            mmd_loss = mmd_loss / opt.batch_size
            recon_loss=F.mse_loss(generated_imgs, labeled_imgs)
            ...
            # 控制台输出loss
            print(
                "[Epoch %d/%d] [Batch %d/%d] [encoder loss: %f] [decoder loss: %f]  [discri_x loss: %f] [classifier loss: %f]"
                % (epoch, opt.n_epochs, i, len(all_dataloader), encoder_loss.item(), decoder_loss.item(),  discriminator_x_loss.item(), classifier_loss.item())
            )
